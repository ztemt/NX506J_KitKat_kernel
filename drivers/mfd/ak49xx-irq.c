...
void ak49xx_unlock_sleep(struct ak49xx *ak49xx)
{
	mutex_lock(&ak49xx->pm_lock);
	if (likely(ak49xx->pm_state == AK49XX_PM_AWAKE))
		ak49xx->pm_state = AK49XX_PM_SLEEPABLE;
	mutex_unlock(&ak49xx->pm_lock);
	wake_up_all(&ak49xx->pm_wq);
}

bool ak49xx_lock_sleep(struct ak49xx *ak49xx) {
	enum ak49xx_pm_state os;
    if (!wait_event_timeout(ak49xx->pm_wq,
        ((os = ak49xx_pm_cmpxchg(ak49xx,AK49XX_PM_SLEEPABLE,AK49XX_PM_AWAKE))== AK49XX_PM_SLEEPABLE ||
            (os == AK49XX_PM_AWAKE)),msecs_to_jiffies(AK49XX_SYSTEM_RESUME_TIMEOUT_MS))) {
        printk("%s: system didn't resume within %dms, s %d, \n",
			__func__,AK49XX_SYSTEM_RESUME_TIMEOUT_MS,ak49xx->pm_state);
		if (likely(ak49xx->pm_state == AK49XX_PM_AWAKE))
			ak49xx->pm_state = AK49XX_PM_SLEEPABLE;
		wake_up_all(&ak49xx->pm_wq);
		return false;
...
static irqreturn_t ak49xx_irq_thread(int irq, void *data)
{
	...
	int i;
	if (unlikely(ak49xx_lock_sleep(ak49xx) == false)) {
		dev_err(ak49xx->dev, "Failed to hold suspend\n");
		return IRQ_NONE;
	}
