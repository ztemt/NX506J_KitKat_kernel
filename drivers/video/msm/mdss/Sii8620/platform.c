...
//mhl_sel_id
	...
	if (ret >= 0) 
		...
		//here  we don't really need to power on and Init 8620
...
int si_8620_pm_suspend(struct device *dev)
{
	...
	printk(KERN_ERR "mhl: si_8620_pm_suspend: before:value:%d\n",gpio_get_value(GPIO_BB_POWER_1V0));
	
	status = down_interruptible(&platform_lock);
	if (status)
		goto done;

	printk(KERN_ERR "mhl: si_8620_pm_suspend: after:value:%d\n",gpio_get_value(GPIO_BB_POWER_1V0));
...
int si_8620_pm_resume(struct device *dev)
{
	...
	gpio_set_value(GPIO_BB_ID_SEL, 0);//1/
