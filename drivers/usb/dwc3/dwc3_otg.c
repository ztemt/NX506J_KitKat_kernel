...
{
	if(dotg_MHL->vbus_otg==NULL){
		dev_err(dotg_MHL->dwc->dev, "%s: returned ;;;;;enable:%d\n", __func__, enable);
		return;
	}
	dev_err(dotg_MHL->dwc->dev, "%s: enable:%d\n", __func__, enable);

	if(enable)
		regulator_enable(dotg_MHL->vbus_otg);
	else
		regulator_disable(dotg_MHL->vbus_otg);
}
void dwc3_otg_start_MHL_power(void)
{
	//struct dwc3_charger *charger = dotg_MHL->charger;
...
#ifdef CONFIG_SII8620_MHL_TX
	dotg_MHL = kzalloc(sizeof(struct dwc3_otg), GFP_KERNEL);
#endif
	if (!dotg) {
...
#ifdef CONFIG_SII8620_MHL_TX
	memcpy(&dotg_MHL,&dotg,sizeof(struct dwc3_otg));
#endif
