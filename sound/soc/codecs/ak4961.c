        ...
        struct snd_ctl_elem_value *ucontrol)
{
    ...
    pr_debug("\n%s  enter.. ak4961_dsp_mode %d,new_dsp_mode %d\n",__func__,ak4961_dsp_mode,new_dsp_mode);

	if (ak4961_dsp_mode == new_dsp_mode) {
        if(new_dsp_mode != DSP_MODE_OFF) {
            pr_debug("ak4961->state = AK4961_DSPRSTNON \n");
            ak4961->state = AK4961_DSPRSTNON;
        }
        pr_debug("ak4961_dsp_mode == new_dsp_mode %d ====\n",ak4961_dsp_mode);
		return 0;
	}
...
//			snd_soc_write(codec, FLOW_CONTROL_3, 0x01);
            if(dsp_sync_domain &0x07)
                snd_soc_update_bits(codec,SYNC_DOMAIN_SELECTOR5,0x07,dsp_sync_domain);
			ak4961->state = AK4961_DSPRSTNON;
		}
	}
...
//Smart PA output 
	...
	/* Power Supply */
	SND_SOC_DAPM_SUPPLY_S("PMSW", 1, FLOW_CONTROL_1, 3, 0,
